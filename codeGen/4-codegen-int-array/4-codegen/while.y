...
program:
    T_PROGRAM T_ID declarations T_BEGIN statements T_END
    {
	    ...
	    if(v.second.decl_type ==array_type)
                std::cout << v.second.label << ": resd 5" << std::endl;
        }
        std::cout << std::endl;
...
|
	...
	symbol_table[*$3] =  var_data( d_loc__.first_line, array_type, new_label() );
	delete $3;	
    }
;
...
|
   T_ID T_SOPEN expression T_SCLOSE T_ASSIGN expression T_SEMICOLON
   {
        ...
        if(symbol_table[*$1].decl_type != array_type || $3->expr_type != integer || $6->expr_type!=integer)
        {
           std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
	...
	"pop ebx\n" +
	$3->expr_code +
	"mov [" + symbol_table[*$1].label + "+4*eax], ebx\n"	
	);
   }
;
...
|
	...
	if($5->expr_type != integer ||  symbol_table[*$3].decl_type != array_type)
	{
	   std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
...
/*|
    T_WRITE T_OPEN T_ID T_SOPEN expression T_SCLOSE T_CLOSE T_SEMICOLON
    {
	if( symbol_table.count(*$3) == 0 )
        {
            std::stringstream ss;
	...
	$$ = new std::string(
	$5->expr_code +
	"push dword[" + symbol_table[*$3].label +"+4*eax]\n" +
	"call write_unsigned\n" +
	"add esp, 4\n"
	);
    }
...
|
   ...
   {
	if(symbol_table[*$1].decl_type != array_type || $3->expr_type!=integer)
	{
           std::stringstream ss;
           ss << d_loc__.first_line << ": Type error." << std::endl;
           error( ss.str().c_str() );
	...
	$3->expr_code +
	"mov eax, [" + symbol_table[*$1].label +"+eax*4"+ "]\n"	
	);
